...
/*
	什么样的数据类型可以使用 nil？
	有 5 种数据类型的零值为 nil：slice、map、chan、interface{}、ptr

	基本数据类型（int、float32、bool、byte、rune、string）和 struct 都不能使用 nil，因为它们的零值不是 nil
...
func main() {
	p := (*P1)(nil)
	fmt.Println(reflect.ValueOf(p).Elem().Interface())
	//fmt.Println(*p)
}
